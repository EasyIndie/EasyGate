...
prepare_tunnel_credentials() {
  ...
  # 凭据已存在时验证其可用性
  if [[ -f "$target" ]]; then
    # 使用临时 config 指向运行时凭据，验证凭据是否匹配 Cloudflare 侧的 tunnel
...
# Write the embedded native demo HTTP server script.
write_native_demo_server() {
  local target="${EASYGATE_HOME}/lib/native-demo-server.py"
  if [[ -f "$target" ]]; then
    return 0
  fi
