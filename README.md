...
3. 在部署设备上安装 Docker、Docker Compose 插件和 `cloudflared` CLI。
4. 使用 `cloudflared` CLI 创建 Cloudflare Tunnel。
5. 配置通配入口：
...
EasyGate 不会自动安装 Docker、Docker Compose 或 `cloudflared`。`make up` 只负责启动项目容器。
