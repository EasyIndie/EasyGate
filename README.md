...
EasyGate 不会安装 Docker。部署脚本在缺少 `cloudflared` CLI 时会下载到运行时目录 `EASYGATE_HOME/bin`，不会写入源码仓库。
...
不需要 clone 源码仓库，直接安装 standalone CLI 并部署：

```sh
curl -fsSL https://raw.githubusercontent.com/EasyIndie/EasyGate/main/scripts/install.sh | bash -s -- deploy --domain example.com
```

macOS 默认安装到 `~/Library/Application Support/EasyGate/bin/easygate`，Linux 默认安装到 `~/.local/share/easygate/bin/easygate`。也可以显式指定运行时目录：
...
Windows 对应维护入口是 `%LOCALAPPDATA%\EasyGate\bin\easygate.ps1`。
...
- 写入运行时 Compose、Traefik 和 cloudflared 配置。
- 复制 tunnel 凭据到运行时目录。
...
```sh
easygate deploy --domain example.com --demo
easygate deploy --domain example.com --port 28080
easygate deploy --domain example.com --skip-route
...
停止并移除 EasyGate 容器和网络，保留 `EASYGATE_HOME` 下的配置和 tunnel 凭据：
