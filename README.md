...
EasyGate 是一个面向家庭 NAT、家庭实验室和一人公司的轻量入口网关。它用 Cloudflare Free + Cloudflare Tunnel 解决公网入口和 HTTPS，用 Traefik 自动发现 Docker 服务，也能接入直接运行在宿主机或局域网其他设备上的服务。

你只需要维护服务名和端口，不需要给每个服务反复配置反向代理、证书和路由器端口转发。
...
- **无需路由器端口转发**：Cloudflare Tunnel 主动出站连接 Cloudflare。
...
- **私有访问保护**：测试服、管理后台和内部工具建议使用 Cloudflare Access。
...
3. 创建 Cloudflare Tunnel，拿到 tunnel token。
4. 在 Cloudflare Tunnel 中添加 public hostname：

   ```text
   *.example.com -> http://traefik:80
   ```

5. 为测试服和管理后台配置 Cloudflare Access，例如：
...
如果你的域名还在普通 DNS 服务商或云厂商 DNS 上，需要先把权威 DNS 切到 Cloudflare。  
迁移含义是“DNS 解析迁移”，不是必须把域名注册商也转走。百度云域名迁移可以参考：[docs/baidu-domain-to-cloudflare.md](docs/baidu-domain-to-cloudflare.md)。
...
脚本会检查 Docker Compose、交互式生成 `.env`、校验配置，并启动 Traefik 和 cloudflared。已有 `.env` 时不会覆盖。
...
- 私有服务必须在 Cloudflare Access 中加登录策略；Traefik labels 和 file provider 只负责路由。
