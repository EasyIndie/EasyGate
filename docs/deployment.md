...
安装后 CLI 在 `~/.easygate/bin/easygate`，自动加入 PATH 并配置 Tab 补全，即装即用。

如需手动激活补全（当前终端立即生效）：
```sh
# bash
source <(easygate completion bash)
# zsh
source <(easygate completion zsh)
```
...
适用于不想安装 Docker 的设备。自动下载 Traefik 二进制，配置写入 `~/.easygate/native/`，启动后台进程并注册系统服务，设备重启后自动恢复：
...
cloudflared 镜像固定为 `2026.6.0`（非 `:latest`）。
